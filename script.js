...
document.addEventListener('DOMContentLoaded', () => {
  ...
  /* ----------------------------------------------------------
     CARROSSEL GENÉRICO
     Inicializa qualquer carrossel passando os IDs dos elementos
  ---------------------------------------------------------- */
  function initCarousel({ trackId, prevId, nextId, dotsId, sectionId }) {
    ...
    function pointerStart(x) { startX = x; dragDist = 0; dragging = true; }
    function pointerMove(x)  { dragDist = x - startX; }
    function pointerEnd() {
      dragging = false;
    ...
    /* Teclas de seta — só quando a seção está visível */
    if (sectionId) {
    ...
    if (sectionId) {
      ...
      if (sec) {
        new IntersectionObserver(entries => {
          entries.forEach(entry => { if (!entry.isIntersecting) pauseAll(); });
  ...
  ---------------------------------------------------------- */
  initCarousel({
    trackId:   'resultadosTrack',
    prevId:    'resultadosPrev',
    nextId:    'resultadosNext',
    dotsId:    'resultadosDots',
